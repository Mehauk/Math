...
class InputNodeOutput(Scene):
    def construct(self):
        ...
        w = MathTex("w_0").scale(1.7).move_to(LEFT*2+UP*0.9)
        w1 = MathTex("w_1").scale(1.7).move_to(LEFT*2+DOWN*0.9)
        line = Line(start=LEFT*3 + RIGHT/3, end=LEFT + UP + RIGHT*0.3)
        line1 = Line(start=LEFT*3 + RIGHT/3, end=LEFT + DOWN + RIGHT*0.3)
        node = Circle(0.7, color=TEAL).move_to(UP)
        node1 = Circle(0.7).move_to(DOWN)
        b = MathTex("b_0").scale(1.7).move_to(UP)
        b1 = MathTex("b_1").scale(1.7).move_to(DOWN)
        line2 = Arrow(start=RIGHT, end=RIGHT*3)
        fx = MathTex("f(x)").scale(2).move_to(RIGHT*4)

        self.add(x, w, w1, line, line1, node, node1, b, b1, line2, fx)