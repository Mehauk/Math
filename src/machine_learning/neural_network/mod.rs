...
impl NeuralNetwork {
    ...
    pub fn load(file_path: &str) -> Result<NeuralNetwork, Box<dyn Error>> {
        ...
        let w = c_split.next().ok_or("File was not properly formatted, or was empty")?;
        let b = c_split.next().ok_or("File was not properly formatted, or was empty")?;

        // split \n
        // split - 
        // take shape and make matrix
        let weigths: Vec<DMatrix<f64>> = w.split(pat)
            .iter()
            .zip(&shape[0..length - 1])
            .map(|(a, b)| DMatrix::<f64>::zeros(*a, *b))
            .collect();
